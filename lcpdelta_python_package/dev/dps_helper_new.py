...
from lcp_delta.enact.signalrcore_wrapper import SignalrClient
import pandas as pd
import threading
import queue
import inspect
...
class DPSHelperNew:
    ...
    def _initialise(self):
        ...
        success = self.signalr_client.start()
        pytime.sleep(1)
    ...
    def _on_close(self):
        ...
        if not self.signalr_client.transport.is_running():
    ...
    def is_connection_alive(self):
        return self.signalr_client and self.signalr_client.transport and self.signalr_client.transport.is_running()
    ...
    def _callback_received(self, m, subscription_id: str, is_for_reconnect: bool = False):
        ...
        self.signalr_client.on(push_name, lambda x: self._process_push_data(x, subscription_id))
    ...
    def _callback_received_multi_series(self, m, handle_data_method, parse_datetimes, is_for_reconnect: bool = False):
        ...
        self.signalr_client.on(push_name, lambda x: self._process_multi_series_push(x, handle_data_method, parse_datetimes))
    ...
    def subscribe_to_epex_trade_updates(self, handle_data_method: Callable[[object], None]) -> None:
        ...
        if hasattr(self, "epex_trade_call_back") and self.epex_trade_call_back:
            self.signalr_client.off(EPEX_SUBSCRIPTION_ID)
